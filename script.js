...
function updatePageNumber() {
  ...
  } else if (currentSlide + 1 === 50) {
    startThirdPageAnmations51();
  } else if (currentSlide + 1 === 55) {
    startThirdPageAnmations56();
  }

  if (currentSlide + 1 >= 5 && currentSlide + 1 <= 12) {
    MappageAnimations();
  }

  if (currentSlide + 1 >= 15 && currentSlide + 1 <= 24) {
    LogoAnimations();
  }

  if (currentSlide + 1 === 26) {
  ...
  const leftContainer = document.querySelector(".right-side");
  ...
  if (currentSlide + 1 === 38) {
    leftContainer.classList.add("animate-part-one");
    ...
    leftContainer.classList.remove("animate-part-one");
  ...
  if (currentSlide + 1 === 39) {
    ...
    partThree.classList.add("animate-part-one");
  } else {
    partOne.classList.remove("animate-part-one");
    partTwo.classList.remove("animate-part-two");
    partThree.classList.remove("animate-part-one");
  }
...
function MappageAnimations() {
  ...
  const rightSectionImages = document.querySelectorAll(
    ".right-section img, .centered-image7 img,.centered-image66 img, .left-img img"
  );
  const leftSections = document.querySelectorAll(
    ".left-section1, .left-section2, .left-sections"
  );
  ...
  leftSections.forEach((leftSection) => {
    ...
    const addressContainers =
      leftSection.querySelectorAll(".address-container");
...
function LogoAnimations() {
  const logos = document.querySelectorAll(
    ".company-gallery .company-item,.company-gallery1 .company-item, .company-gallery2 .company-item, .company-gallery3 .company-item, .company-gallery4 .company-item"
  );
...
function updateSlider() {
  ...
  if (currentSlide == 30) {
    ...
    heading1.style.animation =
      "typing 3s steps(150) 1 normal forwards, blink 1s step-end infinite";

    setTimeout(() => {
      heading2.style.visibility = "visible";
      heading2.style.animation =
        "typing 3s steps(150) 1 normal forwards, blink 1s step-end infinite";
    }, 2700);
  }
...
const groupOfCopmaniesIntentButton = document.querySelectorAll(
...
const texts = [
  "Product <br> Development <br> Guide ",
...
const Img1 = document.getElementById("questiond");
const Img2 = document.getElementById("questiond1");
const Img3 = document.getElementById("questiond2");
const Img4 = document.getElementById("questiond3");
const Img5 = document.getElementById("questiond4");
...
const observer = new IntersectionObserver(
  (entries, observer) => {
    entries.forEach((entry) => {
      const video = document.getElementById("autoPlayVideo1");
...
objectContainer.addEventListener("load", function () {
  ...
  const objectDoc =
    objectContainer.contentDocument || objectContainer.contentWindow.document;
...
const productData = {
  "1-Airlock": `<p class="popupHeader">Airlock India</p>
    <ul>
...
//       // 🔍 Log active slide on init
//       const activeSlide = this.slides[this.activeIndex];
//       console.log('✅ Init - Active Slide Index:', this.activeIndex);
//       console.log('✅ Init - Active Slide Element:', activeSlide);
//     },
//     slideChangeTransitionEnd: function () {
//       const input = document.getElementById('slide-pno10new');
//       updateSlideNumber(this.activeIndex, this.slides.length, input);

//       // 🔍 Log active slide on slide change end
//       const activeSlide = this.slides[this.activeIndex];
//       console.log('✅ Slide Change - Active Slide Index:', this.activeIndex);
//       console.log('✅ Slide Change - Active Slide Element:', activeSlide);
//     }
//   }
// });
// --------------------------
...
// 👇 Function to update reveal classes
function updateSlideContent(swiperInstance) {
  // Remove revealed class from all slides
  document.querySelectorAll(".reveal").forEach((el) => {
    el.classList.remove("revealed");
  });

  const activeSlide = swiperInstance.slides[swiperInstance.activeIndex];
  const revealEl = activeSlide.querySelector(".reveal");

  if (revealEl) {
    revealEl.classList.add("revealed"); // this will trigger animation
  }

  console.log("🔵 Active slide index:", swiperInstance.activeIndex);

  if (revealEl) {
    const children = revealEl.children;
...
// ===============================
// // -----------------------
// swiper10.on('slideChangeTransitionEnd', function () {
//   document.querySelectorAll('.page44leftcon').forEach(el => {
//     el.classList.remove('reveal');
//   });

//   const activeSlide = document.querySelector('.swiper-slide-active');
//   const contentBox = activeSlide.querySelector('.page44leftcon');
//   if (contentBox) {
//     contentBox.classList.remove('reveal');
//     void contentBox.offsetWidth; // reflow
//     contentBox.classList.add('reveal');
//   }
// });
// swiper10.on('slideChangeTransitionEnd', function () {
//   // Remove 'reveal' class from all
//   document.querySelectorAll('.page44leftcon').forEach(el => {
//     el.classList.remove('reveal');
//   });

//   // Add 'reveal' only to active slide
//   const activeSlide = document.querySelector('.swiper-slide-active');
//   const contentBox = activeSlide.querySelector('.page44leftcon');
//   if (contentBox) contentBox.classList.add('reveal');
// });
...
// function triggerLineReveal(swiper) {
//   swiper.slides.forEach((slide, index) => {
...
//       } else {
//         reveal.classList.remove('active-line');
//       }
//     }
//   });
// }
...
const swiper11 = new Swiper(".venting-swiper11", {
  ...
  on: {
    init: function () {
      const input = document.getElementById("slide-pno11");
      updateSlideNumber(this.activeIndex, this.slides.length, input);
      revealOnlyActiveSlide(this); // ✅ Trigger reveal for first slide
    },
    slideChangeTransitionEnd: function () {
      const input = document.getElementById("slide-pno11");
      updateSlideNumber(this.activeIndex, this.slides.length, input);
      revealOnlyActiveSlide(this); // ✅ Reveal only after transition finishes
    },
  },
});
...
function revealOnlyActiveSlide(swiper) {
  swiper.slides.forEach((slide, index) => {
    const reveal = slide.querySelector(".reveal");
    if (reveal) {
      if (index === swiper.activeIndex) {
...
modelViewer.querySelectorAll(".Hotspot").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    const content = hotspotDescriptions[slotName] || {
      title: "",
      text: "",
      audio: "",
    };
...
//     const content = slide.querySelector('.page44leftcon');
//     if (content) content.classList.remove('reveal');
//   });

//   const activeSlide = document.querySelector('.swiper-slide-active');
//   const activeContent = activeSlide.querySelector('.page44leftcon');
//   if (activeContent) activeContent.classList.add('reveal');
// });
...
//   const activeContent = activeSlide.querySelector('.page44leftcon');
//   if (activeContent) {
//     activeContent.classList.add('reveal');
//   }
// });
...
//   // Apply scrollbar styles only for Firefox
//   reveals.forEach(el => {
//     el.style.scrollbarWidth = 'thin'; // or '10px' but Firefox uses 'thin' / 'auto'
//     el.style.scrollbarColor = '#00aaff #ffffff';
//   });
// }
// Detect Firefox
if (navigator.userAgent.toLowerCase().includes("firefox")) {
  document.querySelectorAll(".reveal").forEach((el) => {
    // Use thin or auto scrollbar
    el.style.scrollbarWidth = "thin"; // Try 'auto' first, or 'thin' if you prefer smaller
    el.style.scrollbarColor = "#00aaff #f0f0f0";
  });
}
// ==================page 29 image full screen=====================




  document.addEventListener("DOMContentLoaded", () => {
    const zoomImg = document.querySelector(".zoomable");
...
//   magnifierImg.onload = () => {
//     magnifierImg.style.width = `${img.naturalWidth * zoom}px`;
//     magnifierImg.style.height = `${img.naturalHeight * zoom}px`;
//   };
...
//     magnifier.style.left = `${window.scrollX + rect.left + x - glassSize}px`;
//     magnifier.style.top = `${window.scrollY + rect.top + y - glassSize}px`;

//     magnifierImg.style.left = `${-zoomX * zoom + glassSize}px`;
//     magnifierImg.style.top = `${-zoomY * zoom + glassSize}px`;
//   });
...
const magnifierVenting = document.getElementById("magnifierVenting");
const magnifierImgVenting = document.getElementById("magnifierImgVenting");
const zoomVenting = 2;

function setupVentingMagnifier(img) {
  magnifierImgVenting.src = img.src;
  magnifierVenting.style.display = "block";

  magnifierImgVenting.onload = () => {
    magnifierImgVenting.style.width = `${img.naturalWidth * zoomVenting}px`;
    magnifierImgVenting.style.height = `${img.naturalHeight * zoomVenting}px`;
  };
  ...
  img.addEventListener("mousemove", (e) => {
    ...
    const glassSize = magnifierVenting.offsetWidth / 2;

    magnifierVenting.style.left = `${window.scrollX + rect.left + x - glassSize}px`;
    magnifierVenting.style.top = `${window.scrollY + rect.top + y - glassSize}px`;

    magnifierImgVenting.style.left = `${-zoomX * zoomVenting + glassSize}px`;
    magnifierImgVenting.style.top = `${-zoomY * zoomVenting + glassSize}px`;
  });

  img.addEventListener("mouseleave", () => {
    magnifierVenting.style.display = "none";
  });
}
